...
def eval_linear(args):
    ...
    with torch.no_grad():
        for inp, _ in val_loader:
            inp = inp.cuda()
            features = model.forward_return_n_last_blocks(inp, args.n_last_blocks, args.avgpool_patchtokens, depths)
            print("Feature shape from model.forward_return_n_last_blocks:", features.shape)
...
def validate_network(val_loader, model, linear_classifier, n, avgpool, depths, args):
    ...
    log_line = ('* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} loss {losses.global_avg:.3f}'
                .format(top1=metric_logger.acc1, top5=metric_logger.acc5, losses=metric_logger.loss))

    print(log_line)
    output_path = os.path.join(args.output_dir, "linear_results.txt")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_weights', default='air_out/output0000-13/checkpoint0055.pth', type=str,
    ...
    parser.add_argument('--data_path', default='data/fgvc/images', type=str)
    parser.add_argument('--num_workers', default=10, type=int, help='Number of data loading workers per GPU.')
    parser.add_argument('--val_freq', default=1, type=int, help="Epoch frequency for validation.")
    parser.add_argument('--output_dir', default="air_out_1/air_out-a-2", help='Path to save logs and checkpoints')

    # Dataset
    parser.add_argument('--zip_mode', type=utils.bool_flag, default=False, help="""Whether or not
        to use zip file.""")

    parser.add_argument('--num_labels', default=100, type=int, help='number of classes in a dataset')
