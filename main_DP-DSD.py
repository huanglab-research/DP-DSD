...
# from cub_test.tn_score import *
from cub_test.cam_utils import *
from cub_test.feature_queue import *
...
def get_args_parser():
    ...
    parser.add_argument('--eval_freq', default=10, type=int, help='Epochs between evaluations')
    ...
    # Dataset
    parser.add_argument('--dataset', default="cub", type=str, help='Pre-training dataset.')
    ...
    # Misc
    parser.add_argument('--data_path', default='data/CUB200/CUB_200_2011', type=str,
                        help='Please specify path to the ImageNet training data.')

    ## 特征队列参数
    parser.add_argument('--queue_size', default=4096, type=int, help='特征队列大小')
    parser.add_argument('--sim_threshold', default=0.85, type=float, help='队列相似度阈值')
    parser.add_argument('--top_k', default=1, type=int, help='检索的相似样本数量')

    parser.add_argument('--region_weight', default=0.3, type=float, help='区域损失的权重')

    parser.add_argument('--use_weight_in_global', action='store_true',
                        help='Apply finerCAM weights on global patch stream')
    parser.add_argument('--use_local_branch', action='store_true', help='Enable local patch weighted loss branch')
    parser.add_argument('--local_loss_weight', type=float, default=1.0, help='Weight for local loss term')

    parser.add_argument('--warmup_queue', default=100, type=int, help='初始化队列大小')
    parser.add_argument('--finercam_mode', default='attn2', choices=['weight', 'patch_contrast', 'grad'])
    parser.add_argument('--cam_gamma', default=0.5, type=float, help='finerCAM gamma for diff weighting')
    parser.add_argument('--lambda_patch', default=1.0, type=float, help='patch-level contrast loss weight')

    ##

    parser.add_argument('--pretrained_weights_ckpt', default='pre_trained-checkpoint/swin_tiny_patch4_window7_224.pth',
                        type=str,
                        help="Path to pretrained weights to evaluate.")
    parser.add_argument('--output_dir', default="air_out/output0-5", type=str,
...
def train_esvit(args):
    ...
    # dataset_val = "data/cub_split/test/"
    dataset_val = "data/fgvc/images/test"
    # dataset_val = "data/Car/images/test"
    ...
    # synchronize batch norms (if any)  同步batchNorm
    ...
    # params_groups = list(params_groups) + [{"params": dino_loss.parameters()}]
    # for name, param in dino_loss.named_parameters():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        output_path = os.path.join(args.output_dir, "r_val_results.txt")

        with open(output_path, "a") as f:
            f.write(
                f"Epoch {epoch}: top1={test_starts['top1']:.4f}, top5={test_starts['top5']:.4f}, mAP={test_starts['mAP']:.4f}\n")
...
@torch.no_grad()
def extract_topk_attention_patches(attn: torch.Tensor,
                                   patch_feats: torch.Tensor,
                                   topk: int = 13,
                                   method: str = 'mean_query'):
    """
    Extract top-k most important patch features based on attntion maps.

    Args:
        attn (Tensor): Self-attention map of shape [B, nHead, N, N]
        patch_feats (Tensor): Patch features of shape [B, N, C]
        topk (int): Number of top important patches to extract
        method (str): Method to compute importance score from attention map
                      Options: 'mean_query', 'sum_query', 'mean_all'

    Returns:
        topk_feats (Tensor): Top-k patch features [B, topk, C]
    ...
    if method == 'mean_query':
        importance = attn.mean(dim=1).mean(dim=1)  # [B, N]
    elif method == 'sum_query':
        importance = attn.sum(dim=1).mean(dim=1)  # [B, N]
    elif method == 'mean_all':
        importance = attn.mean(dim=1).mean(dim=2)  # [B, N]
    else:
        raise ValueError(f"Unknown importance method: {method}")

    topk_scores, topk_indices = torch.topk(importance, k=topk, dim=1)  # [B, topk]

    B, N, C = patch_feats.shape
    batch_indices = torch.arange(B).unsqueeze(1).to(patch_feats.device)  # [B,1]
    topk_feats = patch_feats[batch_indices, topk_indices]  # [B, topk, C]

    if is_single:
        return topk_feats.squeeze(0), topk_indices.squeeze(0), topk_scores.squeeze(0), importance.squeeze(0),
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _, image_id) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        it = len(data_loader) * epoch + it  # global training iterationn  全局迭代步数
...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch, targets_mixup, dis_weight):
        """
...
class DDINOLoss(nn.Module):
    def __init__(self, out_dim, ncrops, warmup_teacher_temp, teacher_temp,
        ...
        self.center_momentum = center_momentum
        self.ncrops = ncrops
        self.register_buffer("center", torch.zeros(1, out_dim))  # 全局中心
        self.register_buffer("center_grid", torch.zeros(1, out_dim))  # 局部中心
        ...
        self.warmup_temp_epochs = warmup_teacher_temp_epochs
        # self.patch_focus_loss = PatchDisLoss(temperature=0.1)
        # self.view_con_loss = ViewConLoss(temperature=0.1)
        self.head = nn.Linear(768, 65536)
        self.avgpool = nn.AdaptiveAvgPool1d(1)

    def forward(self, student_output, teacher_output, epoch, targets_mixup,
                patch_features=None, patch_attentions=None, topk_indices=None, warmup_epoch=80, max_beta=0.5):
        """
        Cross-entropy between softmax outputs of the teacher and student networks.
        patch_weights: Tensor of shape [2, B, N], one for each of the two teacher views.
        """

        s_cls_out, s_region_out, s_fea, s_npatch = student_output
        t_cls_out, t_region_out, t_fea, t_npatch = teacher_output

        # === Teacher sharpening & centering ===
        temp = self.teacher_temp_schedule[epoch]
        ...
        N = t_npatch[0]  # num of patches per view
        ...
        # s_split_size_bs = [392, 392, 72, 72, 72, 72, 72, 72, 72, 72]
        ...
        for iq, q in enumerate(t_cls):  # teacher 的第 iq 个 view（通常是两个）
            ...
            for v in range(len(s_cls)):  # 遍历 student 的所有 view（如 10 个）
                ...
                # === view-level classification loss ===
                loss = 0.5 * torch.sum(-q * F.log_softmax(s_cls[v], dim=-1), dim=-1)

                # === region-level loss ===
                s_region_cur = s_region[v].view(B, s_split_size[v], -1)  # [B, T_s, K]
                s_fea_cur = s_fea[v].view(B, s_split_size[v], -1)  # [B, T_s, P]
                t_region_cur = t_region[iq].view(B, N, -1)  # [B, N, K]
                t_fea_cur = t_fea[iq].view(B, N, -1)  # [B, N, P]

                region_sim_matrix = torch.matmul(
                    F.normalize(s_fea_cur, dim=-1),
                    F.normalize(t_fea_cur, dim=-1).permute(0, 2, 1)
                )  # [B, T_s, N]
                region_sim_ind = region_sim_matrix.max(dim=2)[1]  # [B, T_s]

                # === gather matched teacher patches ===
                t_indexed_region = torch.gather(
                    t_region_cur,
                    dim=1,
                    index=region_sim_ind.unsqueeze(-1).expand(-1, -1, t_region_cur.shape[2])
                )  # [B, T_s, K]

                loss_grid = torch.sum(- t_indexed_region * F.log_softmax(s_region_cur, dim=-1),
                                      dim=[-1]).mean(-1)  # B x T_s x K --> B
        ...
        total_loss += 0.1 * loss_pseudo_view

        self.update_center(t_cls_out, t_region_out, patch_output)

        return total_loss

    @torch.no_grad()
    def update_center(self, teacher_output, teacher_grid_output, patch_output):
        """
        Update center used for teacher output.
        """
        ...
        batch_center_patch = torch.sum(patch_output, dim=0, keepdim=True)
        dist.all_reduce(batch_center_patch)
        batch_center_patch = batch_center_patch / (len(patch_output) * dist.get_world_size())
